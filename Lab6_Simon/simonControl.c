...
uint8_t userFailFlag = FALSE;
// this is incremented until it reaches YAY_EXPIRED
uint8_t yayCounter = FALSE;
// this is incremented until it reaces YAY_EXPIRED
uint8_t endCounter = FALSE;
// this is incremented until it reaces YAY_EXPIRED
uint8_t newCounter = FALSE;
...
uint8_t simonFlag = FALSE;
// this flag is used to tell if we need to regenerate a sequence
uint8_t nextFlag = FALSE;

uint8_t passedLevel = FALSE;
...
void simonControl_debugPrint() {
    switch (controlState) {
    case init_st:
        printf("init_st\n\r");
        break;
    case wait_for_untouch_st:
        printf(" wait_for_untouch_st\n\r");
        break;
    case begin_st:
        printf(" begin_st\n\r");
        break;
    case enable_flash_st:
        printf("enable_flash_st\n\r");
        break;
    case wait_for_flash_st:
        printf("wait_for_flash_st\n\r");
        break;
    case disable_flash_st:
        printf("disable_flash_st\n\r");
        break;
    case enable_verify_st:
        printf("enable_verify_st \n\r");
        break;
    case wait_for_verify_st:
        printf(" wait_for_verify_st\n\r");
        break;
    case disable_verify_st:
        printf("disable_verify_st \n\r");
        break;
    case print_end_game_st:
        printf("print_end_game_st \n\r");
        break;
    case wait_end_game_st:
        printf("wait_end_game_st \n\r");
        break;
    case print_yay_st:
        printf("print_yay_st\n\r");
        break;
    ...
    case print_new_level_st:
        printf("print_new_level_st\n\r");
        break;
    case wait_new_level_st:
        printf("wait_new_level_st\n\r");
        break;
...
void simonControl_tick() {
    ...
    case init_st:
        ...
        if (!initFlag) {
            ...
            if (!simonFlag) {
                ...
                display_setCursor(SIMON_MSG_X_COORDINATE,
                        SIMON_MSG_Y_COORDINATE);
                display_setTextSize(SIMON_MSG_TEXT_SIZE);
                display_println(SIMON_MSG);
                display_setCursor(TOUCH_TO_START_MSG_X_COORDINATE,
                        TOUCH_TO_START_MSG_Y_COORDINATE);
    ...
    case print_end_game_st:
        ...
        display_setCursor(GAME_OVER_MSG_X_COORDINATE,
                TOUCH_TO_START_MSG_Y_COORDINATE);
        display_setTextSize(SMALL_MSG_TEXT_SIZE);
        display_println(GAME_OVER_MSG);
        display_setCursor(LEVEL_MSG_X_COORDINATE,
                TOUCH_TO_START_MSG_Y_COORDINATE);
        if (passedLevel)
            display_println(currentLevel);
        else
            display_println(currentLevel - 1);
        break;
    case wait_end_game_st:
