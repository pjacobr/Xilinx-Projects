...
uint8_t verifyEnableFlag = FALSE;
// a flag to determine if the state machine is finished
uint8_t completeFlag = FALSE;
...
void verifySequence_debugPrint()
{
    switch(verifyState){
    case init_st:
        printf("************init_st\n\r");
...
void verifySequence_tick(){
    ...
    case verify_region_st:
        ...
        if (globals_getSequenceValue(index1) != buttonHandler_getRegionNumber()){
            userErrorFlag = 1;
        }
        break;
    case increment_st:
    ...
    case increment_st:
        ...
        if (index1 == (globals_getSequenceIterationLength())){
